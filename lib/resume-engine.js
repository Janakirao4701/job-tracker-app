  ...
  parseExperience: function(raw, docx, FONT, SZ_JOB, SZ_BODY, SZ_BASE, TEXT_W, COLOR_HEAD, COLOR_DARK, COLOR_MID, COLOR_LIGHT) {
    ...
    for (const line of lines) {
      ...
      if (t.startsWith('-') || t.startsWith('•') || t.startsWith('*')) {
        ...
        const jobTitle = parts[2] || parts[0] || ''; // Handle both orders
        const company = parts[0] || parts[1] || '';
        const location = parts[1] || '';
        const dateRange = parts[3] || parts[1] || '';

        // Row 1: COMPANY, LOCATION (Left) | DATE (Right)
        result.push(new Paragraph({
          spacing: { before: 160, after: 20 },
          tabStops: [{ type: TabStopType.RIGHT, position: TEXT_W }],
          children: [
            new TextRun({ text: `${company.toUpperCase()}${location ? ', ' + location : ''}`, bold: true, font: FONT, size: SZ_BODY, color: COLOR_HEAD }),
            new TextRun({ text: '\t' }),
            new TextRun({ text: dateRange, font: FONT, size: SZ_BASE, italics: true, color: COLOR_MID }),
          ]
        }));

        // Row 2: JOB TITLE (Bold)
        if (jobTitle) {
          result.push(new Paragraph({
            spacing: { before: 0, after: 40 },
            children: [new TextRun({ text: jobTitle, bold: true, font: FONT, size: SZ_JOB, color: COLOR_DARK })]
          }));
        }
      } else {
