...
window.ResumeEngine = {
  parseContent: function(text) {
    ...
    // Flexible regex for different label styles (Summary, Skills, Experience)
    const sMatch = text.match(/(?:\[SUMMARY\]|SUMMARY:|PROFESSIONAL SUMMARY:|##\s*Summary)([\s\S]*?)(?=\[|SUMMARY:|SKILLS:|EXPERIENCE:|##|$)/i);
    const kMatch = text.match(/(?:\[SKILLS\]|SKILLS:|TECHNICAL SKILLS:|##\s*Skills)([\s\S]*?)(?=\[|SUMMARY:|SKILLS:|EXPERIENCE:|##|$)/i);
    const xMatch = text.match(/(?:\[EXPERIENCE\]|EXPERIENCE:|PROFESSIONAL EXPERIENCE:|WORK EXPERIENCE:|##\s*Experience)([\s\S]*?)(?=\[|SUMMARY:|SKILLS:|EXPERIENCE:|##|$)/i);
    ...
    // Heuristic Fallback: If no sections were found, treat the whole text as summary/experience
    if (!sections.summary && !sections.skills && !sections.experience) {
      if (text.length < 1000) {
